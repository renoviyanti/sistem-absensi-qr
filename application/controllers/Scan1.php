...
class Scan1 extends Ci_Controller
{
	...
	function cek_id()	{
			$user = $this->user;
			$result_code = $this->input->post('id_komunitas');
			$tgl=date('Y-m-d');
			$jam_klr=date('h:i:s');
			$jam_msk=date('h:i:s');
			$cek_id = $this->Scan_model->cek_id($result_code);
			$cek_abs_klr = $this->Scan_model->kom_abs_klr($result_code,$tgl);
				if (!$cek_id){
					?>
					<script> alert ('Data tidak ditemukan , harap memasukan QR CODE sesuai nomer komunitas masing-masing!');</script> <?php
						$url = base_url().'scan1';
					...
					$this->Scan_model->kom_abs_klr($result_code,$tgl);
					$url = base_url().'scan1';
					...
					header("refresh:1;url=$url");
				 	...
				 	<script> alert ('kehadiran telah di input');</script> <?php
					  $this->Scan_model->kom_abs_msk($result_code);
						$url = base_url().'scan1';
